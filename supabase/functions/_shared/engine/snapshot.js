 ...
 */
export function captureSnapshot(entries, options) {
    ...
    for (const e of entries) {
        ...
        if (!e.sourceId.trim())
            throw new RangeError(`A ${e.kind} entry has no sourceId`);
        ...
        if (existing) {
            if (stable(existing.payload) !== stable(e.payload)) {
                throw new RangeError(`${e.kind} ${e.sourceId} was captured twice with different content. `
                    + 'The library was read twice and gave two answers; keeping one silently would hide that.');
            }
            continue;
        }
        seen.set(key, { ...e, payload: structuredClone(e.payload) });
    }
    const captured = [...seen.values()].sort((a, b) => (a.kind + a.sourceId).localeCompare(b.kind + b.sourceId));
    return {
        id: options.id,
...
export class SnapshotIntegrityError extends Error {
    ...
    expected;
    actual;
    constructor(snapshotId, expected, actual) {
        super(`Snapshot ${snapshotId} does not match its digest (expected ${expected}, computed ${actual}). `
...
export class SnapshotMissingEntryError extends Error {
    kind;
    sourceId;
    constructor(kind, sourceId) {
        super(`The snapshot holds no ${kind} with id ${sourceId}. `
 ...
 * entirely plausible.
 */
export function resolveFromSnapshot(snapshot, kind, sourceId) {
    const entry = snapshot.entries.find((e) => e.kind === kind && e.sourceId === sourceId);
    if (!entry)
        throw new SnapshotMissingEntryError(kind, sourceId);
    return structuredClone(entry.payload);
}
export function snapshotHas(snapshot, kind, sourceId) {
 ...
 */
export function compareSnapshotToLive(snapshot, live, options) {
...
export function driftDelta(entry, field) {
