 ...
 */
export function calculateCrewCost(crew, shifts) {
    ...
    for (const member of crew.members) {
        ...
        if (hoursEachPerShift > 16) {
            warnings.push(`${member.classification.classification} is scheduled ${hoursEachPerShift} hr/shift; verify the shift plan.`);
        }
    ...
    return {
        ...
        lines,
        derivation: `${headcount} workers x ${crew.shiftHours} hr x ${roundTo(shifts, 2)} shifts = ${roundHours(totalLaborHours)} man-hr; ` +
 ...
 */
export function resolveEquipmentRate(candidates, asOf) {
    ...
    let winner = pool[0];
    for (const c of pool) {
        if (EQUIPMENT_RATE_PRECEDENCE.indexOf(c.source) < EQUIPMENT_RATE_PRECEDENCE.indexOf(winner.source)) {
            winner = c;
        }
    }
    assertNonNegative(winner.hourlyRate, 'hourlyRate');
    if (winner.source === 'global_seed') {
        warnings.push('Equipment is priced from the GrounUp global seed rate. Replace it with a company or vendor rate before issuing.');
    }
    ...
    return {
        ...
        derivation: `${winner.source} rate ${unitRate(winner.hourlyRate)}/hr selected` +
 ...
 */
export function calculateEquipmentCost(items, operatingHoursPerUnit, fuelPricePerGallon, defPricePerGallon = 0) {
    ...
    for (const item of items) {
        ...
        if (item.mobilizationRequired && !item.mobilizationCost) {
            warnings.push(`${item.name} is flagged as requiring mobilization but carries no mobilization cost; the move is unpriced.`);
        }
        for (const w of item.rate.warnings)
    ...
    return {
        ...
        lines,
        derivation: `${roundHours(totalEquipmentHours)} equipment-hr; ownership ${ownershipCost}` +
...
export function fuelGallons(operatingHours, gallonsPerHour) {
