 ...
 */
export function evaluateQuantity(input) {
    ...
    const primary = () => {
        const t = peek();
        if (!t)
            return fail('The expression stops before it is finished', expression.length);
        if (t.kind === 'number') {
            pos += 1;
            return t.value;
        }
        ...
        if (t.kind === 'rparen')
            return fail('A bracket was closed that was never opened', t.at);
 ...
 */
export function looksCalculated(input) {
