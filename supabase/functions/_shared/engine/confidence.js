...
import { METHOD_RELIABILITY } from './quantity.js';
import { SOURCE_RELIABILITY } from './production.js';
export function verificationStatus(checks, hasUnresolvedConflict = false) {
    ...
    if (!checks.primarySource)
        return passed === 0 ? 'do_not_price' : 'low_confidence';
    if (passed === 3)
        return 'verified';
    if (passed === 2)
...
export function scoreConfidence(input) {
    ...
    const checkPoints = (input.checks.primarySource ? 4 : 0) +
    ...
    factors.push({
        ...
        effect: checkPoints,
        detail: `primary ${input.checks.primarySource ? 'yes' : 'no'}, cross-source ${input.checks.crossSource ? 'yes' : 'no'}, ` +
    ...
    const arithmetic = workings === ''
        ...
        : `${workings} = ${score}`;
    const explanation = score >= EXPLANATION_FLOOR
...
export function confidenceBand(score) {
    if (score >= CONFIDENCE_THRESHOLDS.autoAcceptFloor)
        return 'verified';
    if (score >= CONFIDENCE_THRESHOLDS.strong)
        return 'strong';
    if (score >= CONFIDENCE_THRESHOLDS.reliable)
        return 'reliable';
    if (score >= CONFIDENCE_THRESHOLDS.assumption)
        return 'assumption';
    if (score >= CONFIDENCE_THRESHOLDS.uncertain)
        return 'uncertain';
    return 'do_not_price';
}
/**
 ...
 */
export function confidenceToContingency(score) {
...
export function requiresSeniorReview(score) {
...
export function evaluateApprovalGate(input) {
    const GATE_ORDER = ['auto_accept', 'estimator_review', 'senior_review', 'rfi_required'];
    const escalations = [];
    ...
    if (input.measurementMethod === 'approximate_scale' ||
    ...
    if ((input.costImpactShare ?? 0) > MAJOR_COST_IMPACT_SHARE) {
        escalations.push({
            gate: 'senior_review',
            reason: `Item carries ${factor((input.costImpactShare ?? 0) * 100)}% of estimate value, above the ` +
    ...
    const gate = escalations.reduce((worst, e) => (GATE_ORDER.indexOf(e.gate) > GATE_ORDER.indexOf(worst) ? e.gate : worst), 'auto_accept');
    const reasons = escalations.length > 0
        ? escalations.map((e) => e.reason)
        : ['Quantity is clearly dimensioned, referenced, conflict-free and scored at or above 95.'];
    const requiredRole = gate === 'auto_accept' ? 'none'
