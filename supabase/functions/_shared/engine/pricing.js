...
export const EMPTY_DIRECT_COST = Object.freeze({
    laborWage: 0,
    laborBurden: 0,
...
export function addDirectCost(a, b) {
    return {
...
export function totalDirectCost(d) {
    return sumMoney([
...
export function applyCostModifiers(d, m) {
    return {
 ...
 */
export function calculatePrice(directCost, indirectCost, profile) {
    ...
    if (regionalFactor !== 1) {
        derivation.push(`x ${factor(regionalFactor)} regional factor (${profile.region ?? 'unspecified region'}) = ${afterRegional}`);
    }
    if (regionalFactor === 1 && profile.region) {
        warnings.push(`Pricing profile names region "${profile.region}" but carries a regional factor of 1.0; ` +
    ...
    if (escalationAdjustment !== 0) {
        derivation.push(`x (1 + ${factor(escalationPercent)})^${escalationYears} escalation = ${adjustedCost}`);
    }
    const components = [...profile.components].sort((a, b) => a.sequence - b.sequence);
    for (const c of components)
        assertNonNegative(c.percent, `markup ${c.code} percent`);
    ...
    const pinnedBasis = (b) => b === 'direct_cost' ? money(directCost * costAdjustment) : money((directCost + indirectCost) * costAdjustment);
    ...
    for (const c of primary) {
        const appliedTo = c.basis === 'profile_default'
        ...
        appliedMarkups.push({
            ...
            amount,
            runningTotal,
            derivation: `${c.label}: ${appliedTo} x ${factor(c.percent)} = ${amount} ` +
    ...
    for (const m of appliedMarkups)
        derivation.push(m.derivation);
    ...
    if (discountAmount > 0) {
        ...
        derivation.push(`discount: ${markedUpTotal} - ${discountAmount}`
        ...
        if (!d?.reason) {
            warnings.push('A discount was applied with no reason recorded. Somebody reviewing this bid cannot '
                + 'tell what was given away or why.');
        }
        if (runningTotal < adjustedCost) {
            warnings.push(`The discount takes the price below cost: ${runningTotal} against ${adjustedCost} of `
    ...
    derivation.push(discountAmount > 0
    ...
    if (totalMarkupPercent > 1) {
        warnings.push(`Markup components total ${factor(totalMarkupPercent * 100)}%, which more than doubles the cost. ` +
 ...
 */
export function compareMarkupMethods(directCost, indirectCost, profile) {
...
export function standardProfile(id, name, method, p, extras = {}) {
    const components = [];
    if (p.overhead)
        components.push({ code: 'OH', label: 'Overhead', percent: p.overhead, basis: 'profile_default', sequence: 10 });
    if (p.profit)
        components.push({ code: 'PROFIT', label: 'Profit', percent: p.profit, basis: 'profile_default', sequence: 20 });
    if (p.contingency)
        components.push({ code: 'CONT', label: 'Contingency', percent: p.contingency, basis: 'profile_default', sequence: 30 });
    if (p.bond)
        components.push({ code: 'BOND', label: 'Bond', percent: p.bond, basis: 'marked_up_total', sequence: 40, disclosed: true });
    if (p.tax)
        components.push({ code: 'TAX', label: 'Tax', percent: p.tax, basis: 'marked_up_total', sequence: 50, disclosed: true });
    return { id, name, method, components, ...extras };
}
/** Round a bid to a presentation increment without ever rounding *down*. */
export function bidRounding(price, increment) {
    assertNonNegative(price, 'price');
    if (increment <= 0)
        return { rounded: money(price), adjustment: 0 };
...
export function parallelTotalPrice(directCost, overhead = 0, profit = 0, contingency = 0, tax = 0) {
    assertNonNegative(directCost, 'directCost');
    return money(directCost * (1 + overhead + profit + contingency + tax));
}
/** Break-even quantity: fixed cost / (unit price - unit variable cost). */
export function breakEvenQuantity(fixedCost, unitPriceValue, unitVariableCost) {
