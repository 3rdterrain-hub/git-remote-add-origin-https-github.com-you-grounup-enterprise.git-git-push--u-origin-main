...
function assertFactor(f, driver) {
    if (!Number.isFinite(f) || f < MIN_FACTOR || f > MAX_FACTOR) {
        throw new RangeError(`${driver} factor must be between ${MIN_FACTOR} and ${MAX_FACTOR}, received ${f}`);
    }
    return f;
}
/** Collapse a scenario's adjustments into one factor per driver. */
function factorsOf(scenario) {
    const out = new Map();
    for (const a of scenario.adjustments) {
        ...
        if (!a.rationale.trim()) {
            throw new RangeError(`${scenario.id}: the ${DRIVER_LABELS[a.driver]} adjustment has no rationale. `
        ...
        if (out.has(a.driver)) {
            ...
            throw new RangeError(`${scenario.id} adjusts ${DRIVER_LABELS[a.driver]} twice. State one factor per driver.`);
        }
 ...
 */
export function applyScenario(input, scenario) {
    ...
    const profile = contingencyFactor === 1 ? next.pricingProfile : {
        ...next.pricingProfile,
        components: next.pricingProfile.components.map((c) => c.code.toLowerCase().includes('conting')
 ...
 * already wrong.
 */
export function priceScenarios(input, scenarios) {
    const bases = scenarios.filter((s) => s.kind === 'base');
    if (bases.length !== 1) {
        throw new ScenarioSetError(`A scenario set needs exactly one base to measure the others against; found ${bases.length}.`);
    }
    const ids = scenarios.map((s) => s.id);
    const duplicate = ids.find((id, i) => ids.indexOf(id) !== i);
    if (duplicate)
        throw new ScenarioSetError(`Duplicate scenario id ${duplicate}.`);
    ...
    if (baseEntry.result.bidPrice !== unadjusted.bidPrice) {
        throw new ScenarioSetError(`The base scenario priced at ${baseEntry.result.bidPrice} but the estimate prices at `
    ...
    const results = priced.map(({ scenario, result }) => {
    ...
    if (low && high && low.bidPrice > high.bidPrice) {
        ...
        warnings.push(`The low scenario (${low.bidPrice}) prices above the high scenario (${high.bidPrice}). `
            + 'Check the direction of the production and calendar efficiency factors.');
    }
    if (basePrice > 0 && spread / basePrice > 0.5) {
        warnings.push(`The scenarios span ${roundTo((spread / basePrice) * 100, 1)}% of the base price. `
 ...
 */
export function analyzeSensitivity(input, options = {}) {
    ...
    return {
        ...
        derivation: [
            ...
            ...ranked.map((e) => `  ${e.label}: ${e.bidPrice} (${e.delta >= 0 ? '+' : ''}${e.delta}, `
