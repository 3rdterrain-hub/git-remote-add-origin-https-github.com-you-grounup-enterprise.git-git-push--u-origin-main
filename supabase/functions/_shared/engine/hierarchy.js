 ...
 */
export const PARAMETRIC_METHOD = 'estimator_allowance';
/**
 ...
 */
export function resolveHierarchy(input) {
    const warnings = [];
    const byId = new Map();
    ...
    for (const n of input) {
        ...
        if (!byId.has(n.parentId)) {
            throw new RangeError(`Line ${JSON.stringify(n.id)} names a parent that is not in this estimate: `
    ...
    const seen = new Set();
    const sortSiblings = (ids) => [...ids].sort((a, b) => {
    ...
    const walk = (id, depth, path) => {
        if (seen.has(id)) {
            throw new RangeError(`The line structure contains a cycle, reached again at ${JSON.stringify(id)}`);
        }
        seen.add(id);
        const n = byId.get(id);
        const parent = n.parentId ? resolved.get(n.parentId) : undefined;
        const kids = sortSiblings(children.get(id) ?? []);
        const basis = n.quantityBasis ?? { kind: 'measured' };
        const nodeWarnings = [];
        let quantity;
        let derivation;
        if (basis.kind === 'per_parent_unit') {
            if (!parent) {
                throw new RangeError(`"${n.description}" is quantified per parent unit and has no parent`);
            }
            if (!(basis.perUnit > 0)) {
                throw new RangeError(`"${n.description}" needs a positive quantity per parent unit`);
            }
            ...
            if (n.measuredQuantity !== undefined && n.measuredQuantity !== quantity) {
                 ...
                 */
                nodeWarnings.push(`"${n.description}" carries a measured quantity of ${n.measuredQuantity} `
        ...
        if (n.parametric) {
            if (kids.length > 0) {
                throw new RangeError(`"${n.description}" is priced at a rate and also has ${kids.length} `
            ...
            if (!n.parametric.basis || n.parametric.basis.trim().length === 0) {
                throw new RangeError(`"${n.description}" is priced at a rate and does not say where the rate `
        ...
        for (const kid of kids)
            walk(kid, depth + 1, [...path, n.description]);
    };
    ...
    if (resolved.size !== byId.size) {
        ...
        throw new RangeError(`${orphaned.length} line(s) are unreachable from any root, which means a cycle: `
 ...
 * appearing here.
 */
export function rollUp(hierarchy, costs) {
    ...
    const nodes = hierarchy.nodes.map((node) => {
        ...
        const mine = own.get(node.id);
        const ownCost = roundTo((mine?.ownCost ?? 0)
