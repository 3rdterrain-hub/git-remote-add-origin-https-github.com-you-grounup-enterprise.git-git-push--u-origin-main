...
import { analyzeBottleneck, analyzeProduction, calculateDuration, resolveModifiers, } from './production.js';
...
export function calculateEstimateLine(input) {
    ...
    if (modifiers.derivation.length)
        derivation.push(`MODIFIERS: ${modifiers.derivation.join(' | ')}`);
    ...
    if (input.productionRate) {
        ...
        if (resources.length > 1) {
            ...
            derivation.push(`CONTROLLING RESOURCE: ${bottleneck.controllingResourceName} at ${bottleneck.operationCapacityPerHour}/hr — ${bottleneck.improvementNote}`);
            if (bottleneck.controllingResourceId !== input.productionRate.id) {
                warnings.push(`Production is governed by ${bottleneck.controllingResourceName}, not the primary spread. ` +
    ...
    if (effectiveProductionPerHour > 0) {
        ...
        warnings.push('No production rate is attached, so crew and equipment hours cannot be derived from quantity. ' +
    ...
    if (input.equipment?.length) {
        equipment = calculateEquipmentCost(input.equipment, operatingHoursPerUnit, input.fuelPricePerGallon ?? 0, input.defPricePerGallon ?? 0);
        ...
        if ((input.fuelPricePerGallon ?? 0) === 0 && equipment.fuelGallons > 0) {
            warnings.push(`${equipment.fuelGallons} gallons of fuel are consumed but no fuel price is set; fuel cost is zero.`);
        }
    }
    // 7. Materials ------------------------------------------------------------
    let materialCost = 0;
    ...
    let haul;
    if (input.haul) {
        const loaderProduction = input.haul.loaderProductionPerHour ?? (effectiveProductionPerHour > 0 ? effectiveProductionPerHour : undefined);
        if (loaderProduction === undefined) {
            warnings.push('A haul was defined but no loading production is available to size the fleet against; haul not priced.');
        }
        else {
...
export function calculateEstimate(input) {
    ...
    for (const l of lines)
        directCost = addDirectCost(directCost, l.directCost);
    const totalDirect = totalDirectCost(directCost);
    // Indirects -------------------------------------------------------------
    const indirectDetail = [];
    ...
    const weightedConfidence = totalForWeighting > 0
        ? roundTo(lines.reduce((a, l) => a + l.confidence.score * l.totalDirectCost, 0) / totalForWeighting, 1)
        : lines.length > 0
    ...
    if (input.contingencyOverride) {
        ...
        warnings.push(`Contingency overridden to ${factor(appliedContingency * 100)}% by ${input.contingencyOverride.reason} ` +
        ...
        if (profileContingency !== null) {
            warnings.push(`Profile contingency of ${factor(profileContingency * 100)}% is below the ` +
    ...
    const effectiveProfile = {
        ...
        components: [
            ...
            ...(appliedContingency > 0
                ? [{ code: 'CONT', label: 'Contingency', percent: appliedContingency, basis: 'profile_default', sequence: 30 }]
                : []),
        ],
    };
    /*
    ...
    const overriddenMarkup = sumMoney(overridden.map((l) => money(l.totalDirectCost * (overrideFor(l.id) ?? 0))));
    if (overridden.length > 0) {
        warnings.push(`${overridden.length} line${overridden.length === 1 ? ' carries its' : 's carry their'} own `
    ...
    const byRemainder = [...flexible].sort((a, b) => (b.price * 100 - Math.floor(b.price * 100)) - (a.price * 100 - Math.floor(a.price * 100)));
    ...
    for (const l of priced)
        approvalSummary[l.approval.gate].push(l.id);
    ...
    for (const l of lines) {
        if (l.warnings.length)
            warnings.push(`Line ${l.id}: ${l.warnings.length} warning(s)`);
    }
    if (input.status === 'issued' || input.status === 'approved') {
        if (blockedFromIssue) {
            warnings.push(`Estimate is marked "${input.status}" while ${approvalSummary.senior_review.length + approvalSummary.rfi_required.length} ` +
...
function executiveDecision(lines, summary, weightedConfidence) {
    ...
    if (summary.rfi_required.length > 0) {
        return {
            ...
            reason: `${summary.rfi_required.length} line(s) depend on information the supplied documents cannot resolve ` +
    ...
    if (summary.senior_review.length > 0) {
        return {
            ...
            reason: `${summary.senior_review.length} line(s) carry a document conflict, a material assumption, a major ` +
    ...
    if (summary.estimator_review.length > 0 || weightedConfidence < 95) {
        ...
        return {
            ...
            reason: `Weighted confidence is ${weightedConfidence}. ${summary.estimator_review.length} line(s) required ` +
