 ...
 * be recalculated.
 */
export function exportEstimate(estimate, options) {
 ...
 */
export function importEstimate(document, options) {
    const problems = [];
    const warnings = [];
    ...
    if (typeof schemaVersion !== 'string' || !/^\d+\.\d+\.\d+$/.test(schemaVersion)) {
        ...
        if (theirs > ours) {
            ...
            problems.push(`The document uses schema ${schemaVersion}, which is newer than this build understands `
    ...
    if (typeof document.engineVersion !== 'string' || !document.engineVersion.trim()) {
        ...
        warnings.push(`The document was produced by engine ${document.engineVersion}; this build is `
    ...
    if (typeof estimate.id !== 'string' || !estimate.id.trim())
        problems.push('estimate.id is missing.');
    if (typeof estimate.name !== 'string' || !estimate.name.trim())
        problems.push('estimate.name is missing.');
    if (typeof estimate.number !== 'string' || !estimate.number.trim())
        problems.push('estimate.number is missing.');
...
export function serializePortable(document) {
...
export function parsePortable(json, options) {
    let parsed;
    try {
        parsed = JSON.parse(json);
    }
    catch (err) {
        throw new PortableImportError([`The file is not valid JSON: ${err.message}`]);
    }
    return importEstimate(parsed, options);
}
/** The line ids in a document, for reconciling an import against what was sent. */
export function portableLineIds(document) {
    return (document.estimate.lines ?? []).map((l) => l.id);
}
