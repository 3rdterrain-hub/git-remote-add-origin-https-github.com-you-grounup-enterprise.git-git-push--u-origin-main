 ...
 */
import { addWorkingDays, assertCalendar, assertIsoDate, epochDay, isoFromEpochDay, isWorkingDay, nextWorkingDay, previousWorkingDay, workingDayDelta, } from './calendar.js';
/**
 ...
 */
const dayAfter = (iso) => isoFromEpochDay(epochDay(iso) + 1);
const dayBefore = (iso) => isoFromEpochDay(epochDay(iso) - 1);
export const DEPENDENCY_TYPES = ['finish_to_start', 'start_to_start', 'finish_to_finish', 'start_to_finish'];
export const CONSTRAINT_TYPES = ['start_no_earlier', 'finish_no_later', 'must_start_on', 'must_finish_on'];
export class ScheduleCycleError extends Error {
    cycle;
    constructor(cycle) {
        super(`The schedule logic contains a cycle: ${cycle.join(' -> ')}. ` +
...
function wholeDays(activity, warnings) {
    const d = activity.durationDays;
    if (!Number.isFinite(d) || d < 0) {
        throw new ScheduleInputError(`Activity ${activity.id} has a duration of ${d}; a duration must be zero or more.`);
    }
    if (Number.isInteger(d))
        return d;
    const rounded = Math.ceil(d);
    warnings.push(`${activity.id} has a duration of ${d} days, which has no finish date on a working-day ` +
 ...
 */
function finishFrom(calendar, start, duration) {
...
function startFrom(calendar, finish, duration) {
    return duration <= 1 ? finish : addWorkingDays(calendar, finish, -(duration - 1));
}
const later = (a, b) => (epochDay(a) >= epochDay(b) ? a : b);
const earlier = (a, b) => (epochDay(a) <= epochDay(b) ? a : b);
function validate(input, warnings) {
    assertIsoDate(input.dataDate);
    if (input.requiredFinish !== undefined)
        assertIsoDate(input.requiredFinish);
    ...
    if (!fallback) {
        throw new ScheduleInputError(`The default calendar ${input.defaultCalendarId} is not among the calendars supplied.`);
    }
    const nodes = new Map();
    for (const activity of input.activities) {
        if (!activity.id)
            throw new ScheduleInputError('Every activity needs an id.');
        ...
        if (!calendar) {
            throw new ScheduleInputError(`Activity ${activity.id} names calendar ${activity.calendarId}, which was not supplied.`);
        }
        if (activity.constraintType && !activity.constraintDate) {
            throw new ScheduleInputError(`Activity ${activity.id} has a ${activity.constraintType} constraint with no date.`);
        }
        if (activity.constraintDate) {
            assertIsoDate(activity.constraintDate);
            if (!activity.constraintType) {
                throw new ScheduleInputError(`Activity ${activity.id} has a constraint date with no constraint type.`);
            }
            if (!CONSTRAINT_TYPES.includes(activity.constraintType)) {
                throw new ScheduleInputError(`Activity ${activity.id} has an unknown constraint type ${activity.constraintType}.`);
            }
        }
    ...
    for (const dep of input.dependencies) {
        ...
        if (!pred) {
            throw new ScheduleInputError(`A dependency names predecessor ${dep.predecessorId}, which is not an activity here.`);
        }
        if (!succ) {
            throw new ScheduleInputError(`A dependency names successor ${dep.successorId}, which is not an activity here.`);
        }
        ...
        if (!DEPENDENCY_TYPES.includes(dep.type)) {
            throw new ScheduleInputError(`The link ${dep.predecessorId} -> ${dep.successorId} has an unknown type ${dep.type}.`);
        }
        const lag = dep.lagDays ?? 0;
        if (!Number.isInteger(lag)) {
            throw new ScheduleInputError(`The link ${dep.predecessorId} -> ${dep.successorId} has a lag of ${lag}; lag is whole working days.`);
        }
        ...
        if (seen.has(key)) {
            throw new ScheduleInputError(`${dep.predecessorId} -> ${dep.successorId} is linked more than once.`);
        }
        seen.add(key);
...
function topologicalOrder(nodes) {
    ...
    while (ready.length) {
        ...
        for (const dep of nodes.get(id).outgoing) {
            ...
            if (left === 0)
                ready.push(dep.successorId);
        }
    }
    if (order.length !== nodes.size)
        throw new ScheduleCycleError(findCycle(nodes));
    return order;
}
...
function forwardPass(nodes, order, input, warnings) {
    for (const id of order) {
        ...
        for (const dep of node.incoming) {
            ...
            switch (dep.type) {
                case 'finish_to_start': {
                    ...
                    if (epochDay(bound) > epochDay(startBound)) {
                        startBound = bound;
                        driver = pred.activity.id;
                    }
                    break;
                }
                case 'start_to_start': {
        ...
        if (finishBound !== null) {
            ...
            if (epochDay(impliedStart) > epochDay(startBound))
                startBound = impliedStart;
        }
        ...
        if (constraintDate && constraintType) {
            switch (constraintType) {
                ...
                case 'must_start_on': {
                    const c = nextWorkingDay(cal, constraintDate);
                    if (epochDay(c) < epochDay(startBound)) {
                        warnings.push(`${id} must start on ${constraintDate}, but its predecessors do not release it until ` +
                ...
                case 'must_finish_on': {
                    ...
                    if (epochDay(implied) < epochDay(startBound)) {
                        warnings.push(`${id} must finish on ${constraintDate}, which requires starting ${implied}, before its ` +
...
function backwardPass(nodes, order, projectFinish, input) {
    for (const id of [...order].reverse()) {
        ...
        for (const dep of node.outgoing) {
            ...
            switch (dep.type) {
                ...
                case 'start_to_start': {
                    const bound = previousWorkingDay(cal, addWorkingDays(sc, previousWorkingDay(sc, succ.lateStart), -lag));
                    startBound = startBound === null ? bound : earlier(startBound, bound);
                    break;
                }
                case 'finish_to_finish': {
                    const bound = previousWorkingDay(cal, addWorkingDays(sc, previousWorkingDay(sc, succ.lateFinish), -lag));
                    finishBound = earlier(finishBound, bound);
                    break;
                }
                case 'start_to_finish': {
 ...
 */
function freeFloat(node, nodes, totalFloat) {
 ...
 */
function longestCriticalChain(nodes, order, critical) {
    const best = new Map();
    for (const id of [...order].reverse()) {
        if (!critical.has(id))
            continue;
        let tail = [];
        for (const dep of nodes.get(id).outgoing) {
            if (!critical.has(dep.successorId))
                continue;
    ...
    for (const id of order) {
        ...
        if (candidate && candidate.length > chain.length)
            chain = candidate;
    }
    return chain;
}
...
export function calculateSchedule(input) {
    ...
    for (const id of order) {
        ...
        activities.push({
            ...
            derivation: `${node.earlyStart} to ${node.earlyFinish} (${span}, calendar ${cal.id}, ${because}); ` +
    ...
    for (const id of order) {
        ...
        if (node.outgoing.length > 0)
            continue;
        if (node.earlyFinish === projectFinish)
            continue; // The real end of the job.
        const float = activities.find((a) => a.id === id).totalFloatDays;
        warnings.push(`${id} has no successor, so its float of ${float} working days is bounded only by the ` +
            'project finish rather than by any work that waits on it. Usually a missing link.');
    }
    let finishFloat = null;
    if (input.requiredFinish) {
        const cal = nodes.get(order[0]).calendar;
        finishFloat = workingDayDelta(cal, nextWorkingDay(cal, projectFinish), nextWorkingDay(cal, input.requiredFinish));
        if (finishFloat < 0) {
            warnings.push(`The schedule finishes ${projectFinish}, ${Math.abs(finishFloat)} working days after the ` +
 ...
 */
export function compareToBaseline(result, baseline, calendar) {
    ...
    for (const activity of result.activities) {
        ...
        const startVariance = workingDayDelta(calendar, nextWorkingDay(calendar, base.plannedStart), nextWorkingDay(calendar, activity.earlyStart));
        const finishVariance = workingDayDelta(calendar, nextWorkingDay(calendar, base.plannedFinish), nextWorkingDay(calendar, activity.earlyFinish));
    ...
    for (const base of baseline.activities) {
        if (current.has(base.activityId))
            continue;
        rows.push({
    ...
    return {
        ...
        activities: rows,
        projectFinishVarianceDays: baselineFinish === null ? null : workingDayDelta(calendar, nextWorkingDay(calendar, baselineFinish), nextWorkingDay(calendar, result.projectFinish)),
        behindCount: behind,
        aheadCount: ahead,
    };
}
