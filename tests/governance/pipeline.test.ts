...
describe('no compiled output shadows a source file', () => {
  ...
  /**
   * The one place compiled JavaScript belongs in a source tree.
   *
   * `supabase/functions/_shared/engine` is the estimating engine compiled for
   * the Edge runtime, generated by `scripts/build-edge-engine.mjs` and held to
   * the source by a drift check in `npm run verify`. It is committed on purpose:
   * a deploy that silently shipped without it would leave the one function that
   * prices estimates unable to price estimates.
   *
   * The exemption is deliberately narrow. It applies only while the directory
   * carries the FINGERPRINT its generator writes — so an accidental `tsc` dump
   * into that path fails this test exactly like any other, and the exemption
   * cannot be inherited by a directory that merely sits beside it.
   */
  const GENERATED = 'supabase/functions/_shared/engine';
  const generatedIsMarked = existsSync(join(ROOT, GENERATED, 'FINGERPRINT'));

  it('marks its one generated directory as generated', () => {
    // If the marker is gone the exemption below stops applying, which is the
    // intended failure rather than a silently widened hole.
    expect(generatedIsMarked, `${GENERATED}/FINGERPRINT is missing`).toBe(true);
  });
  ...
  it('leaves no emitted javascript in any source tree', () => {
    ...
    for (const root of roots) {
      for (const file of walk(root)) {
        ...
        if (generatedIsMarked && file.startsWith(GENERATED)) continue;
