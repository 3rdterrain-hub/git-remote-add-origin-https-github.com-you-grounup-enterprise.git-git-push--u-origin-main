...
| **Database** | `supabase/migrations` | 173 migrations, 171 tables, 20 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 171 tables, with `app.assert_security_gates()` run at the end
