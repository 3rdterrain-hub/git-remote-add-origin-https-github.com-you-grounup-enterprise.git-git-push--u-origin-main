...
| **Database** | `supabase/migrations` | 123 migrations, 167 tables, 19 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 167 tables, with `app.assert_security_gates()` run at the end
