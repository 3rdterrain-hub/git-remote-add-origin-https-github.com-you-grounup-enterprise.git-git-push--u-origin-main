...
| **Database** | `supabase/migrations` | 81 migrations, 150 tables, 18 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 150 tables, with `app.assert_security_gates()` run at the end
