...
| **Database** | `supabase/migrations` | 66 migrations, 142 tables, 17 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 142 tables, with `app.assert_security_gates()` run at the end
