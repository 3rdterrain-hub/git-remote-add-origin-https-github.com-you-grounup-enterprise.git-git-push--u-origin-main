...
| **Database** | `supabase/migrations` | 64 migrations, 141 tables, 16 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 141 tables, with `app.assert_security_gates()` run at the end
