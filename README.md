...
| **Database** | `supabase/migrations` | 83 migrations, 153 tables, 18 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 153 tables, with `app.assert_security_gates()` run at the end
