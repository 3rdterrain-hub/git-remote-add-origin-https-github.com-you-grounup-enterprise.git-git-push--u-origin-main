...
| **Database** | `supabase/migrations` | 63 migrations, 139 tables, 16 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 139 tables, with `app.assert_security_gates()` run at the end
