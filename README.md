...
| **Database** | `supabase/migrations` | 80 migrations, 149 tables, 18 reporting views: multi-tenant schema, RLS forced on every table, audit ledger, governance triggers. |
...
- **Tenant isolation** is enforced by PostgreSQL row level security, `FORCE`d on
  every one of the 149 tables, with `app.assert_security_gates()` run at the end
