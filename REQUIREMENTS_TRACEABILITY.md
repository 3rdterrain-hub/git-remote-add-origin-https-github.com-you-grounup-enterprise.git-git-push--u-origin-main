...
| O-031 | A task's category says production or support, not what trade it is | All 8,532 shipped tasks carry one of two categories — Support (7,161) and Production (1,371) — so the Tasks tab cannot be navigated by trade the way Services can (5 real categories). Not a code defect: the seed has no trade on a task, and inventing one is forbidden | Decide whether a task gets a real category/subcategory, and seed or map it |
