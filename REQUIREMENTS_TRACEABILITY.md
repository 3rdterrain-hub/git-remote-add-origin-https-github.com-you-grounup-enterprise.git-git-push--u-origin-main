...
| C-063 | A condition of your own, and proof one reaches the machine | owner: "Yes I want it… inside the estimator conditions there's no way to apply that through the excavator" | high | libraries, estimating | migration 0228, `AddCondition`, four tests over the pricing joint | `a-condition-you-can-describe-yourself.test.ts` (10), `estimate-pricing.test.ts` (57) | tested |
| C-062 | Every library tab can create a row | owner: "create control — assemblies and pricing profiles" | high | libraries | migrations 0226–0227, `AddAssembly`, `AddPricingProfile` | `an-assembly-and-a-profile-you-can-start.test.ts` (9) | tested |
| C-061 | A machine and a production rate can be created | owner: "equipment, tasks and production rates still have no way to create one" | high | libraries | migrations 0224–0225, `AddEquipment`, `AddProductionRate` | `a-machine-and-a-rate-you-can-add.test.ts` (11) | tested |
| C-060 | Every library figure is editable where it is shown | owner: "we need to be able to set a base wage, a burden… I can't change the unit… hourly, daily, weekly, monthly… make it editable, clickable, customizable" | high | libraries | migrations 0221–0223, `TextCell`, `UnitAndCost`, `AddLaborRate`, wired across the tabs | `a-library-row-you-can-actually-change.test.ts` (16) | tested |
| C-059 | Every library tab can put a row away, shipped rows included | owner: "it has to be on all relevant categories" | high | libraries | migrations 0219–0220, `library_hidden_rows`, `ArchiveAction` on all twelve tabs | `nothing-in-a-library-is-destroyed.test.ts` (13), `archive-action.test.tsx` (10) | tested |
| C-058 | Nothing in a library is destroyed, and anything archived comes back | owner: "be an archive is cool, it's better than deleted. But it has to be on all relevant categories" | high | libraries | migrations 0217–0218 `set_library_status`, `components/library/archive-action.tsx`, a Show archived switch, five readers parameterized | `nothing-in-a-library-is-destroyed.test.ts` (8), `archive-action.test.tsx` (10) | tested |
| C-057 | A task says what trade it belongs to | owner: "what is task category showing" | medium | libraries | migration 0216 `my_library_tasks`, `loadTasks`, the Trade column and filter | `a-task-knows-what-trade-it-belongs-to.test.ts` (7) | tested |
| C-056 | A lead form you can send, not only paste | owner, repeatedly since 4 Sep 2026: "want to funnel leads through the system and have a lead form to capture leads" | high | CRM | `pages/lead-form.tsx` at `/lead/:key`, `submitLead` in `lib/data/lead-forms.ts`, the link on the form card | `lead-form.test.tsx` (11), live submission against the deployed project | tested |
| C-055 | The library shows every row it holds | owner: "i dont see all the services in the library" | high | libraries | `everyRow` paging in `lib/data/library.ts`, all ten readers | `library-pages.test.ts` (6), `a-library-is-not-truncated.test.ts` (3) | tested |
| C-054 | The platform can say whether its AI credential actually works | owner: "check whether the AI actually works now" | high | observability | `credentialCheck` in `_shared/observability/health.ts`, `GET /health/ai` | `observability.test.ts` (54) | tested |
| C-053 | Global search reads records, and finds them | owner: "fix the global search so it reads records" | high | search | migrations 0214–0215, `lib/search.ts`, `app-shell.tsx` | `a-search-that-finds-the-thing-you-named.test.ts` (15), `search.test.ts` (7) | tested |
| C-051 | Company Settings: editable, clickable, collapsible, with Billing and API Access inside it | owner, 13 Sep 2026; toolbar order | high | settings | migrations 0211–0213, `lib/data/team.ts`, `components/settings/{team-members,company-roles,audit-ledger}.tsx`, `settings.tsx` tabs | `a-company-you-can-administer.test.ts` (22), `team-and-roles.test.tsx` (13), `settings.test.tsx` (11) | tested |
| C-052 | The confidence thresholds a screen shows are the ones that route | this session — three copies of the same four numbers | high | engine | `CONFIDENCE_THRESHOLDS` in `packages/engine/src/confidence.ts`; duplicate `confidenceBandOf`/`contingencyFor` deleted from `estimate.ts` | `settings.test.tsx` asserts the screen against the constants; engine suite (708) unchanged | tested |
...
| O-030 | The Anthropic API key on the project is refused | `GET /health/ai` reports `the provider refused this credential`; the one AI job ever attempted (16 Sep) failed `401 authentication_error / API key is invalid`. Present but not valid — so no AI feature has ever produced a row | The owner sets a working key: `npx supabase secrets set ANTHROPIC_API_KEY=...`, then re-check `/health/ai` |
